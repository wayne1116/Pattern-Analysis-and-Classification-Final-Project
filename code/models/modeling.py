...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        for layer_block in self.layer:
            ...
            layer_count += 1
            if layer_count == 12:
                certain_hidden_states = hidden_states

        encoded = self.encoder_norm(hidden_states)
        certain_hidden_states = self.encoder_norm(certain_hidden_states)
        return encoded, attn_weights, certain_hidden_states
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        encoded, attn_weights, certain_hidden = self.encoder(embedding_output)
        return encoded, attn_weights, certain_hidden
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        self.bbox_regression = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(int(config.hidden_size/2), 4)
        )

    def forward(self, x, labels=None, bbox=None):
        x, attn_weights, certain_hidden = self.transformer(x)
        logits = self.head(certain_hidden[:, 0])
        result = self.bbox_regression(certain_hidden[:, 0])

        if labels is not None:
            # classification loss
            loss_fct = CrossEntropyLoss()
            loss1 = loss_fct(logits.view(-1, self.num_classes), labels.view(-1))
            # regression loss (multi-task)
            loss_reg = nn.SmoothL1Loss()
            loss2 = loss_reg(result.float(), bbox.float())
            # loss_bce = nn.BCELoss()
            # loss2 = loss_bce(result.float(), foregorund.float())
            return loss1 + loss2 * 3
